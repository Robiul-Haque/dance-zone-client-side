...
const Dashboard = () => {
    ...
    useEffect(() => {
        if (user?.email) {
            fetch(`https://dance-zone-server.vercel.app/if-exist-admin/${user?.email}`)
                .then(res => res.json())
                .then(data => setVerifyAdmin(data))
        }

        setInterval(() => setTime(new Date()), 1000)
    }, [user, navigation, userLogout]);
    ...
    return (
        <div className="drawer lg:drawer-open">
            ...
            <div className="flex flex-col items-center justify-center drawer-content m-4">
                {/* Page content here */}
                <label htmlFor="my-drawer-2" className="my-5 btn btn-primary drawer-button lg:hidden">Open Menu</label>
                <div className="my-10">
                    {
                        verifyAdmin?.role === "admin" && <Online><Outlet></Outlet></Online>
                    }
                    <Offline>
