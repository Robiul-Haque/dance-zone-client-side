...
const Navbar = () => {
    ...
    useEffect(() => {
        fetch(`https://summer-camp-backend-rho.vercel.app/if-exist-student/${user?.email}`)
            .then(res => res.json())
    ...
    return (
        <div className="navbar bg-base-200 md:px-60">
            <div className="navbar-start">
                <Link to={'/'} className="font-bold text-2xl font-serif block md:block lg:hidden">Dance Zone</Link>
                <div className="dropdown">
                    <label tabIndex={0} className="btn btn-ghost  lg:hidden">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h8m-8 6h16" /></svg>
                    </label>
                    <ul tabIndex={0} className="menu menu-sm dropdown-content mt-3 p-4 shadow bg-base-100 rounded-box w-36 z-40">
                        <li>
                            <NavLink to='/' className='font-medium text-base block text-center mt-3'>Home</NavLink>
                        </li>
                        <li>
                            <NavLink to='/course' className='font-medium text-base block text-center my-3'>Course</NavLink>
                        </li>
                        <li>
                            <NavLink to='/instructor' className='font-medium text-base block text-center'>Instructor</NavLink>
                        </li>
                        <li>
                            <NavLink to='/contact-us' className='font-medium text-base block text-center my-3'>Contact us</NavLink>
                        </li>
                    </ul>
                </div>
                <Link to={'/'} className="font-bold text-2xl font-serif hidden md:hidden lg:block">Dance Zone</Link>
            </div>
            ...
            <div className="navbar-end hidden justify-end gap-4 md:hidden lg:flex">
                {
                    user?.email ?
                        <>
                            {
            ...
            <div className="dropdown">
                ...
                <ul tabIndex={0} className="menu menu-sm dropdown-content mt-3 p-4 shadow bg-base-100 rounded-box w-40 z-40">
                    {
                        user?.email ?
                            <>
                                <li className="mb-4">
            ...
            {
                user?.email && <div className="avatar online block lg:hidden ml-2">
